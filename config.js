...
<div id="app" class="flex min-h-screen hidden">
  ...
  <aside class="sidebar fixed left-0 top-0 h-screen flex flex-col z-[200]">
    ...
    <div class="p-4 border-t border-dark-600/30">
      ...
      <button onclick="refreshData()" class="mt-3 w-full text-xs text-brand-400 hover:text-brand-300 flex items-center gap-1.5 justify-center py-1.5 rounded-lg hover:bg-brand-500/5 transition-all font-medium">
...
async function fetchMetaDailyInsights(adAccountId, dateStart, dateEnd) {
  ...
  try {
    const url = `${META_BASE}/${actId}/insights?fields=spend,impressions,clicks,actions,cpc,cpm,ctr,frequency,outbound_clicks&time_range={"since":"${dateStart}","until":"${dateEnd}"}&time_increment=1&level=account&access_token=${META_ACCESS_TOKEN}`;
...
async function fetchGregConfig() {
  try {
    ...
    rows.forEach(row => {
      ...
      const mode = row.c?.[2]?.v?.toString().trim().toUpperCase();
      if (!type || !name || !mode) return;
      if (type === 'manager' && ['HARD','SOFT','OFF'].includes(mode)) {
        gregConfig.managerModes[name] = mode;
      } else if (type === 'account' && ['HARD','SOFT','OFF'].includes(mode)) {
        gregConfig.accountModes[name] = mode;
      }
    });
...
// ═══════════════════════════════════════════════════════════════
//  SMART LIVE REFRESH — silent poll + diff + surgical re-render
// ═══════════════════════════════════════════════════════════════
//
//  Instead of calling loadAllData() (which re-renders everything,
//  resets scroll, flashes the page), this system:
//    1. Fetches fresh data silently in the background
//    2. Computes a fingerprint of the old vs new data
//    3. Only re-renders the CURRENT VIEW if data actually changed
//    4. Preserves scroll position throughout
//    5. Shows a subtle dot pulse while fetching, no loading screen
//
...
function startLiveRefresh(intervalMs) {
  ...
  const interval = intervalMs || 90000; // default 90 seconds
  console.log(`[LiveRefresh] Started — polling every ${interval / 1000}s`);

  _liveRefreshTimer = setInterval(async () => {
    if (_liveRefreshBusy) {
      console.log('[LiveRefresh] Skipped — previous fetch still running');
      return;
    }
    try {
      _liveRefreshBusy = true;
      await liveRefreshCycle();
    } catch (e) {
      console.warn('[LiveRefresh] Error:', e);
    } finally {
...
function stopLiveRefresh() {
  if (_liveRefreshTimer) {
    ...
    console.log('[LiveRefresh] Stopped');
  }
}

// Build a quick fingerprint string for an array of objects
// Used to detect if data actually changed without deep comparison
function dataFingerprint(arr) {
  if (!arr || !arr.length) return '0';
  // Hash key fields that change most often: billing, goals, leads, spend
  let hash = arr.length.toString();
  for (let i = 0; i < arr.length; i++) {
    const o = arr[i];
    hash += '|' + (o.name || o.account || '') +
            (o.bookedAppts || '') +
            (o.amountSpent || '') +
            (o.goodToBill || '') +
            (o.billed || '') +
...
async function liveRefreshCycle() {
  console.log('[LiveRefresh] Fetching fresh data…');

  // Show subtle status indicator (yellow dot, no loading screen)
  ...
  // Snapshot old fingerprints BEFORE fetching
  const oldAccountsFP = dataFingerprint(allAccounts);
  const oldCyclesFP = dataFingerprint(allCycles);
  const oldLeadsFP = dataFingerprint(allLeads);

  try {
    // ── Fetch all data silently (same as loadAllData but no UI work) ──
    const podNames = Object.keys(SHEETS);
    const [podResults, leadResults, adIdMaps] = await Promise.all([
      Promise.all(podNames.map(name => fetchSheetData(name, SHEETS[name]))),
      Promise.all([
    ...
    const newAccounts = podResults.flatMap(r => r.accounts);
    const newCycles = podResults.flatMap(r => r.cycles);
    const newLeads = leadResults.flat();

    // Merge CSV-derived ad account IDs
    const combinedAdIdMap = new Map(adIdMaps.flatMap(m => [...m]));
    newAccounts.forEach(acct => {
      if (!acct.adAccountId) {
        const csvId = combinedAdIdMap.get(acct.name);
        if (csvId) acct.adAccountId = csvId;
      }
      (acct.cycles || []).forEach(c => {
        if (!c.adAccountId && acct.adAccountId) c.adAccountId = acct.adAccountId;
      });
    ...
    const newAccountsFP = dataFingerprint(newAccounts);
    const newCyclesFP = dataFingerprint(newCycles);
    const newLeadsFP = dataFingerprint(newLeads);

    const accountsChanged = (newAccountsFP !== oldAccountsFP);
    const cyclesChanged = (newCyclesFP !== oldCyclesFP);
    const leadsChanged = (newLeadsFP !== oldLeadsFP);
    const anythingChanged = accountsChanged || cyclesChanged || leadsChanged;

    if (!anythingChanged) {
      console.log('[LiveRefresh] No changes detected — skipping re-render');
      if (statusDot) statusDot.className = 'w-2 h-2 rounded-full bg-green-500';
      return;
    }

    console.log(`[LiveRefresh] Changes detected — accounts:${accountsChanged} cycles:${cyclesChanged} leads:${leadsChanged}`);
    ...
    if (statusText) statusText.textContent = `${allAccounts.length} accounts · ${allLeads.length} leads`;

    // ── Surgical re-render: only re-render the CURRENT view, preserve scroll ──
    const mainContent = document.querySelector('.main-content');
    const scrollTop = mainContent ? mainContent.scrollTop : 0;

    const savedNav = JSON.parse(localStorage.getItem('nav_state') || 'null');
    const view = currentView || (savedNav && savedNav.view) || 'dashboard';

    switch (view) {
      case 'dashboard':    renderDashboard(); break;
      case 'pod':          if (currentPod) renderPodView(currentPod); break;
      case 'manager':      if (currentManager) renderManagerView(currentManager); break;
      case 'account':
        if (savedNav && savedNav.param) renderAccountDetail(savedNav.param.name, savedNav.param.adAccountId);
        break;
    ...
    console.log('[LiveRefresh] View re-rendered successfully');

  } catch (e) {
    console.warn('[LiveRefresh] Fetch error (will retry next cycle):', e.message);
  }

  // Restore green status dot
  if (statusDot) statusDot.className = 'w-2 h-2 rounded-full bg-green-500';
}
      ...
      <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex items-center gap-4">
          ${(() => {
            ...
            if (hs === null) return '<div class="w-14 h-14 rounded-2xl bg-dark-700/50 flex items-center justify-center text-dark-500 text-lg font-bold border border-dark-600">—</div>';
            const hc = healthScoreColor(hs);
            return '<div class="w-14 h-14 rounded-2xl flex items-center justify-center text-lg font-extrabold shadow-lg" style="background:'+hc.bg+';color:'+hc.text+';border:2px solid '+hc.text+'22;">'+hs+'</div>';
          })()}
          <div>
            <div class="flex items-center gap-3">
        ...
        <div class="flex items-center gap-2 flex-shrink-0 flex-wrap justify-end">
          <button onclick="document.getElementById('quick-edit-panel').classList.toggle('hidden')" class="flex items-center gap-1.5 px-3 md:px-4 py-2 md:py-2.5 rounded-xl text-xs md:text-sm font-semibold text-dark-200 hover:text-white bg-dark-700/60 hover:bg-dark-700/90 border border-dark-600/40 transition-all">
      ...
      return `<div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-5">
        ...
        <div class="glass rounded-xl p-4">
          <div class="flex items-center justify-between mb-2">
            <div class="text-dark-400 text-[10px] uppercase tracking-wider">Greg's Lead CPL Goal</div>
            <button onclick="document.getElementById('greg-goal-edit').classList.toggle('hidden')" class="text-[9px] text-dark-500 hover:text-brand-400 transition-colors" title="Edit Greg Goal">⚙️ Adjust</button>
          </div>
          ${gregCPL !== null ? `
          ...
          <div id="greg-goal-edit" class="hidden mt-3 pt-3 border-t border-dark-600/40">
            <label class="text-[10px] text-dark-500">Greg Goal (Booked Appointments) — Override</label>
            <div class="text-[9px] text-dark-400 mt-0.5 mb-1.5">This is a <strong class="text-dark-300">booked appointment target</strong> used to squeeze performance. If an account is projected well above its normal goal, raise this to judge ad sets against the higher bar. Greg derives a lead CPL threshold from this value × the L2B rate.</div>
            ...
            <div class="text-[9px] text-red-400/70 mt-1">⚠ DANGEROUS — Setting this too high will aggressively pause underperforming ad sets. Only raise when an account is clearly outperforming its cycle goal.</div>
          </div>
        </div>
      </div>`;
    })()}

      <!-- Quick Edit: Hidden by default, toggled by Edit Cycle button -->
      <div id="quick-edit-panel" class="glass rounded-xl p-4 mb-4 hidden">
        <div class="text-dark-400 text-[10px] uppercase tracking-wider mb-3">Quick Edit — Current Cycle</div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div>
            <label class="text-[10px] text-dark-500">CPA Goal</label>
            <input id="qe-cpa-goal" type="number" step="0.01" value="${activeCyc.cpaGoal||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Booked Goal</label>
            <input id="qe-booked-goal" type="number" value="${activeCyc.bookedGoal||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Cycle Length (days)</label>
            <input id="qe-cycle-length" type="number" value="${activeCyc.cycleStartDate && activeCyc.cycleEndDate ? Math.round((parseLocalDate(activeCyc.cycleEndDate) - parseLocalDate(activeCyc.cycleStartDate)) / 86400000) : 28}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Daily Budget</label>
            <input id="qe-daily" type="number" step="0.01" value="${activeCyc.dailyBudget||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Monthly Budget</label>
            <input id="qe-monthly" type="number" step="0.01" value="${activeCyc.monthlyBudget||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Cycle Start</label>
            <input id="qe-cycle-start" type="date" value="${activeCyc.cycleStartDate||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
          <div>
            <label class="text-[10px] text-dark-500">Cycle End</label>
            <input id="qe-cycle-end" type="date" value="${activeCyc.cycleEndDate||''}" class="w-full bg-dark-700/80 border border-dark-600/50 rounded-lg text-xs text-white px-2 py-1.5 focus:outline-none focus:border-brand-500" />
          </div>
        </div>
        <div class="text-[9px] text-dark-500 mt-1.5 mb-1">Changing cycle length auto-adjusts end date from start date</div>
        <button onclick="quickEditCycle('${esc(name)}','${esc(activeCyc.cycle)}')" class="mt-1 w-full py-1.5 rounded-lg text-xs font-semibold text-white bg-brand-500/20 hover:bg-brand-500/30 border border-brand-500/30 transition-all">Save Changes</button>
      </div>
...
// Modes: 'HARD' (pauses ads), 'SOFT' (Slack alerts only), 'OFF' (disabled)
const gregConfig = {
  managerModes: {},    // { 'Cole': 'HARD', 'Tyler': 'SOFT', ... }
  accountModes: {},    // { 'Some Account': 'OFF' } — per-account overrides
  defaultMode: 'SOFT'  // Fallback for managers not listed
};
...
function getGregMode(accountName, managerName) {
  if (gregConfig.accountModes[accountName]) return gregConfig.accountModes[accountName];
  return gregConfig.managerModes[managerName] || gregConfig.defaultMode;
}

async function setManagerGregMode(manager, mode) {
  gregConfig.managerModes[manager] = mode;
  renderAdminView();

  if (APPS_SCRIPT_URL) {
    const result = await writeToSheet('setManagerGregMode', { manager, mode });
    if (result.ok) {
      showToast(`Greg → ${mode} for all of ${manager}'s accounts ✓ Saved to Sheet`, mode === 'HARD' ? 'success' : mode === 'SOFT' ? 'warning' : 'info');
    } else {
      showToast(`⚠️ Greg set to ${mode} locally, but failed to save to Sheet`, 'error');
    }
  } else {
    showToast(`Greg → ${mode} for all of ${manager}'s accounts (local only)`, mode === 'HARD' ? 'success' : mode === 'SOFT' ? 'warning' : 'info');
  }
}

async function setAccountGregMode(accountName, mode) {
  const acct = allAccounts.find(a => a.name === accountName);
  if (!acct) return;
  const managerMode = gregConfig.managerModes[acct.manager] || gregConfig.defaultMode;
  if (mode === managerMode) {
    delete gregConfig.accountModes[accountName];
  } else {
    gregConfig.accountModes[accountName] = mode;
  }
  renderAdminView();

  const sendMode = mode === managerMode ? null : mode;
  if (APPS_SCRIPT_URL) {
    const result = await writeToSheet('setAccountGregMode', { name: accountName, mode: sendMode });
    if (result.ok) {
      const msg = sendMode === null
        ? `${accountName}: Greg reset to manager default (${managerMode}) ✓ Saved`
        : `${accountName}: Greg → ${mode} ✓ Saved to Sheet`;
    ...
    const msg = sendMode === null
      ? `${accountName}: Greg reset to manager default (${managerMode}) (local only)`
      : `${accountName}: Greg → ${mode} (local only)`;
    showToast(msg, mode === 'HARD' ? 'success' : mode === 'SOFT' ? 'warning' : 'info');
  }
}
  ...
  el.innerHTML = `
    ...
    <div class="glass rounded-2xl p-5 mb-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-${Math.min(managers.length, 5)} gap-3">
        ${managers.map(m => {
          const mode = gregConfig.managerModes[m] || gregConfig.defaultMode;
          const acctCount = allAccounts.filter(a => a.manager === m).length;
          ...
          <div class="bg-dark-800/60 rounded-xl p-3 border border-dark-600/30">
            ...
            <div class="flex rounded-lg overflow-hidden border border-dark-600/40">
              ${['OFF','SOFT','HARD'].map(md => `<button onclick="setManagerGregMode('${esc(m)}','${md}')" class="flex-1 py-1.5 text-[10px] font-bold transition-all ${mode === md
                ? md === 'HARD' ? 'bg-green-500/30 text-green-300' : md === 'SOFT' ? 'bg-yellow-500/30 text-yellow-300' : 'bg-dark-600/60 text-dark-300'
                : 'text-dark-500 hover:text-dark-200 hover:bg-dark-700/40'}">${md}</button>`).join('')}
            </div>
      ...
      <table class="w-full text-sm min-w-[480px] md:min-w-0">
        ...
        <tbody>
          ...
          <tr class="border-b border-dark-600/20 hover:bg-dark-700/30 transition-all ${idx % 2 === 0 ? '' : 'bg-dark-800/20'}">
            ...
            <td class="px-3 md:px-4 py-3 text-center">
              <div class="inline-flex rounded-lg overflow-hidden border ${hasOverride ? 'border-brand-500/40' : 'border-dark-600/30'}" title="${hasOverride ? 'Account override (manager default: '+gregConfig.managerModes[a.manager]+')' : 'Using manager default'}">
                ${['OFF','SOFT','HARD'].map(m => `<button onclick="setAccountGregMode('${esc(a.name)}','${m}')" class="px-2 py-1 text-[10px] font-bold transition-all ${effectiveGregMode === m
...
function renderSettingsTab(managers) {
  const hasGAS = !!APPS_SCRIPT_URL;
  const cfg = slackConfigCache || { webhooks: {}, billingAdmin: '', notifyToggles: { reports: {}, warnings: {} } };
  const toggles = (cfg.notifyToggles) || { reports: {}, warnings: {} };
  const savedBillingAdmin = localStorage.getItem('roofignite_billing_admin') || cfg.billingAdmin || (CONFIG.SLACK && CONFIG.SLACK.DEFAULT_BILLING_ADMIN) || '';
  ...
  return `
    ...
    <div class="glass rounded-2xl p-6 mb-6">
      <div class="flex items-center gap-3 mb-2">
        ...
        <div>
          <h3 class="text-base font-bold text-white">Slack Integrations</h3>
          <p class="text-dark-400 text-xs mt-0.5">Billing notifications are sent to specific team members via Slack webhooks</p>
        </div>
      </div>

      <!-- Billing Admin Dropdown -->
      <div class="bg-dark-800/60 rounded-xl p-4 border border-dark-600/30 mt-4 mb-4">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-semibold text-white">Billing Admin</p>
            <p class="text-[11px] text-dark-400 mt-0.5">Receives <span class="text-yellow-400 font-medium">Good to Bill</span> and <span class="text-orange-400 font-medium">Try Again</span> notifications</p>
          </div>
          <select id="billing-admin-select" onchange="saveBillingAdmin(this.value)" class="bg-dark-900/80 border border-dark-600/50 rounded-xl text-sm text-white px-4 py-2 focus:outline-none focus:border-brand-500 min-w-[160px]">
      ...
      <div class="space-y-3 mb-4">
        <p class="text-xs font-semibold text-dark-300 uppercase tracking-wider">Webhook URLs</p>
        ${managers.map(m => {
          const isConfigured = cfg.webhooks && cfg.webhooks[m];
          return `
          <div class="bg-dark-800/60 rounded-xl p-3.5 border border-dark-600/30 flex items-center gap-3">
            ...
            <div class="flex-1 relative">
              <input id="slack-webhook-${m.replace(/\s/g,'_')}" type="password" placeholder="https://hooks.slack.com/services/..." value="${isConfigured ? '••••••••••••••••••••' : ''}"
              ...
              <button onclick="const inp=document.getElementById('slack-webhook-${m.replace(/\s/g,'_')}');inp.type=inp.type==='password'?'text':'password'" class="absolute right-2 top-1/2 -translate-y-1/2 text-dark-400 hover:text-white">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              </button>
            </div>
            <button onclick="saveSlackWebhook('${m}')" class="px-3 py-1.5 rounded-lg text-[10px] font-semibold bg-brand-500/20 text-brand-400 border border-brand-500/30 hover:bg-brand-500/30 transition-all">Save</button>
            <span class="text-[10px] font-medium ${isConfigured ? 'text-green-400' : 'text-dark-500'} min-w-[80px] text-right">${isConfigured ? '✅ Connected' : '⚪ Not set'}</span>
          </div>`;
        }).join('')}
      </div>
      ...
      <div class="flex items-center justify-between">
        ...
        <p class="text-[10px] text-dark-500 max-w-xs text-right">Account managers receive <span class="text-green-400">Billed</span> &amp; <span class="text-red-400">Failed Payment</span> notifications on their own webhook.</p>
      </div>
    </div>
    ...
    <div class="glass rounded-2xl p-6 mb-6">
      <div class="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p class="text-dark-400 text-xs mt-0.5">Enable or disable Daily Reports and Cycle End Warnings per manager</p>
        </div>
      </div>

      <div class="space-y-3 mt-4">
        <div class="grid grid-cols-[1fr_100px_100px] gap-2 px-3">
          <span class="text-[10px] font-semibold text-dark-300 uppercase tracking-wider">Manager</span>
          <span class="text-[10px] font-semibold text-dark-300 uppercase tracking-wider text-center">Daily Reports</span>
          <span class="text-[10px] font-semibold text-dark-300 uppercase tracking-wider text-center">Cycle Warnings</span>
        </div>
        ${managers.map(m => {
          const reportsOn = toggles.reports[m] !== false;
          const warningsOn = toggles.warnings[m] !== false;
          return `
          <div class="bg-dark-800/60 rounded-xl p-3.5 border border-dark-600/30 grid grid-cols-[1fr_100px_100px] gap-2 items-center">
            <span class="text-sm font-medium text-white">${m}</span>
            <div class="flex justify-center">
              <button onclick="toggleSlackNotify('${m}','reports',${!reportsOn})" class="relative w-11 h-6 rounded-full transition-colors duration-200 ${reportsOn ? 'bg-green-500' : 'bg-dark-600'}" title="${reportsOn ? 'Enabled' : 'Disabled'}">
                <span class="absolute top-0.5 ${reportsOn ? 'left-[22px]' : 'left-0.5'} w-5 h-5 rounded-full bg-white shadow transition-all duration-200"></span>
              </button>
            </div>
            <div class="flex justify-center">
              <button onclick="toggleSlackNotify('${m}','warnings',${!warningsOn})" class="relative w-11 h-6 rounded-full transition-colors duration-200 ${warningsOn ? 'bg-green-500' : 'bg-dark-600'}" title="${warningsOn ? 'Enabled' : 'Disabled'}">
                <span class="absolute top-0.5 ${warningsOn ? 'left-[22px]' : 'left-0.5'} w-5 h-5 rounded-full bg-white shadow transition-all duration-200"></span>
              </button>
            </div>
          </div>`;
        }).join('')}
    ...
    <div class="glass rounded-2xl p-6 mb-6">
      ...
      <details class="group">
        <summary class="cursor-pointer text-xs font-semibold text-dark-300 hover:text-white transition-colors flex items-center gap-2">
        ...
        <div class="mt-3 text-sm text-dark-200 space-y-3">
...
manager | Cole | HARD
manager | Tyler | SOFT
account | Some Account | OFF</div>
          <p class="text-[11px] text-dark-400">The Greg script reads this tab instead of hardcoded OWNER_RUN_FLAGS.</p>

          <p><strong class="text-white">2. Deploy the Apps Script</strong> (Extensions → Apps Script → Deploy → Web app)</p>
          <p class="text-[11px] text-dark-400">Set "Execute as" to <strong>Me</strong> and "Who has access" to <strong>Anyone</strong>. Copy the Web app URL and paste it above.</p>

          <p><strong class="text-white">3. Paste the full backend code</strong> from your appscripts.gs file into the Apps Script editor.</p>
        </div>
      </details>
    </div>
  `;
...
async function saveSlackWebhook(name) {
  const inputId = 'slack-webhook-' + name.replace(/\s/g, '_');
  const inp = document.getElementById(inputId);
  if (!inp) return;
  const url = inp.value.trim();
  if (url.startsWith('••')) { showToast('Click the field and paste a new URL first', 'error'); return; }
  if (!APPS_SCRIPT_URL) { showToast('Connect Apps Script first', 'error'); return; }

  const result = await writeToSheet('saveSlackWebhook', { person: name, webhookUrl: url });
  if (result.ok) {
    showToast(`Webhook ${url ? 'saved' : 'removed'} for ${name}`, 'success');
...
async function testSlackWebhooks() {
  if (!APPS_SCRIPT_URL) { showToast('Connect Apps Script first', 'error'); return; }
  showToast('Sending test messages...', 'success');
  const result = await writeToSheet('testSlackWebhook', {});
  if (result.ok) {
    showToast(`Test sent to ${result.count || 'all configured'} webhook(s)`, 'success');
  } else {
    showToast('Test failed — check webhook URLs', 'error');
  }
}
...
async function toggleSlackNotify(manager, type, enabled) {
  ...
  if (result.ok) {
    ...
    if (!slackConfigCache) slackConfigCache = { webhooks: {}, billingAdmin: '', notifyToggles: { reports: {}, warnings: {} } };
    if (!slackConfigCache.notifyToggles) slackConfigCache.notifyToggles = { reports: {}, warnings: {} };
    if (type === 'reports') {
      if (enabled) { delete slackConfigCache.notifyToggles.reports[manager]; }
      else { slackConfigCache.notifyToggles.reports[manager] = false; }
    } else {
      if (enabled) { delete slackConfigCache.notifyToggles.warnings[manager]; }
      else { slackConfigCache.notifyToggles.warnings[manager] = false; }
    }
    showToast(`${type === 'reports' ? 'Daily Reports' : 'Cycle Warnings'} ${enabled ? 'enabled' : 'disabled'} for ${manager}`, 'success');
    renderAdminView();
  } else {
...
async function createNewManager() {
  ...
  // Update local state
  gregConfig.managerModes[name] = 'SOFT';
...
async function createNewClient() {
  ...
  const cycle1 = {
    ...
    accountManager: mgr, notes: '', goodToBill: '', billed: '', billingNotes: ''
  };

  const newAcct = {
    name, manager: mgr, pod, adAccountId: adId || '', section: '',
    isPaused: false, status: 'Q1 Onboarded',
    bookedGoal, gregGoal: bookedGoal, cpaGoal: null, dailyBudget, monthlyBudget: dailyBudget ? dailyBudget * 28 : null, cycles: [cycle1]
  };
...
async function quickEditCycle(accountName, cycleName) {
  ...
  const cycleLength = document.getElementById('qe-cycle-length')?.value;
  ...
  // Handle cycle length → auto-adjust end date from start
  if (cycleStart) c.cycleStartDate = cycleStart;
  if (cycleEnd) c.cycleEndDate = cycleEnd;
  if (cycleLength && c.cycleStartDate) {
  ...
  if (APPS_SCRIPT_URL) {
    ...
    if (result.ok) {
      ...
      showToast(`⚠️ Updated locally, but failed to save to Sheet`, 'error');
    }
  } else {
    showToast(`Updated ${c.cycle} for ${accountName} (local only)`, 'success');
  }
...
async function executeAddCycle(accountName) {
  ...
  const newCycle = {
    ...
    accountManager: acct.manager, notes: '', goodToBill: '', billed: '', billingNotes: ''
  };
...
function showToast(message, type = 'info') {
  ...
  toast.className = 'fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:bottom-6 z-[300] flex items-center gap-3 px-5 py-3 rounded-xl shadow-2xl text-white text-sm font-medium bg-gradient-to-r ' + (colors[type] || colors.info);
  toast.style.cssText = 'animation: slideIn 0.3s ease-out; opacity: 0; transform: translateY(20px);';
...
account | Some Account Name | OFF</div>
  ...
  // Greg mode changes → write to "Greg Config" tab
  if (action === 'setManagerGregMode' || action === 'setAccountGregMode') {
    ...
    for (let i = 1; i &lt; vals.length; i++) {
      if (vals[i][0] === type &amp;&amp; vals[i][1] === name) {
        if (mode === null) { configSheet.deleteRow(i + 1); } // remove override
        else { configSheet.getRange(i + 1, 3).setValue(mode); }
        found = true; break;
      }
    }
    if (!found &amp;&amp; mode !== null) configSheet.appendRow([type, name, mode]);
  }
...
function onAuthSuccess(user, freshLogin) {
  ...
  // Load dashboard data, then start smart live refresh every 90s
  loadAllData().then(() => startLiveRefresh(90000));
}
